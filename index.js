...
let player = { 
  ...
  dir: 'bas', 
  moving: false,
  argent: 100 // Nouveau paramètre
};

// On définit les objets avec une fonction d'interaction
let tree = { x: 60, y: 60, interact: () => {
...
let windows = { 
  ...
  interact: () => {
    ...
    if (keyp(5)) { // btnp = Appui unique sur E
      if (player.argent >= 50) {
        player.argent -= 50;
        // ON CHANGE LE TILE !
...
function TIC() {
  ...
  // 1. INPUTS
  if (btn(0)) { dy -= player.speed; player.dir = 'haut';   player.moving = true; }
  else if (btn(1)) { dy += player.speed; player.dir = 'bas';    player.moving = true; }
  else if (btn(2)) { dx -= player.speed; player.dir = 'gauche'; player.moving = true; }
  else if (btn(3)) { dx += player.speed; player.dir = 'droite'; player.moving = true; }

  // 2. COLLISION & MOUVEMENT
  let next = { x: player.x + dx, y: player.y + dy };
  let hit = false;
  for (let c of colliders) if (isBlocking(next, c)) hit = true;
  if (!hit) { player.x = next.x; player.y = next.y; }

  // 3. ANIMATION SPRITES
  let frame = Math.floor(time() / 150) % 2;
  let spriteId = 259; 
  if (player.dir === 'bas')    spriteId = player.moving ? (frame == 0 ? 257 : 259) : 259;
  else if (player.dir === 'haut')   spriteId = player.moving ? (frame == 0 ? 323 : 325) : 327;
  else if (player.dir === 'gauche') spriteId = player.moving ? (frame == 0 ? 297 : 299) : 329;
  else if (player.dir === 'droite') spriteId = player.moving ? (frame == 0 ? 295 : 293) : 331;
